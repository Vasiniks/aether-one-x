...
function findSegment(p: number): { a: FilmKey; b: FilmKey; t: number } {
  if (p <= KEYS[0].at) return { a: KEYS[0], b: KEYS[0], t: 0 }
  for (let i = 1; i < KEYS.length; i++) {
    const b = KEYS[i]
    if (p <= b.at) {
      const a = KEYS[i - 1]
      return { a, b, t: smoothstep((p - a.at) / (b.at - a.at)) }
    }
  }
...
export function sampleFilm(p: number): FilmSample {
  ...
  POS_CURVE.getPoint(p, OUT.pos)
  TGT_CURVE.getPoint(p, OUT.target)
